/*
    Suppose that there will never be more than one character of pushback.
     ...
     Same program as p4-5.c
    Add access to library functions like sin , exp , and pow .
    See <math.h> in Appendix B, Section 4.

    Modified getch & ungetch
*/

#include <stdio.h>      //include standard func like printf, scanf
#include <stdlib.h>     //includes exit()
#include <ctype.h>      //includes isalpha, isdigit
#include <math.h>       //includes fmod, sin, exp, pow
#include <string.h>     //includes strcmp func

//used to identify what type of input
#define NUMBER      0
#define MATHFUNC    1

//maximum stack and array sizes
#define MAXOP 100
#define STACKMAXCAPACITY 100
#define BUFSIZE 1

//Global variables as they are shared between functions
int bufp = 0;
char buf;
int stackPointer = 0;
...
int getop(char localBuffer[])
{
    ...
    */
    //To check if it some math function
    if(isalpha(c))
    {
        while(isalpha(c))
        {
...
//to duplicate the top most element
void Duplicate(void)
{
	double t1 = pop();
	push(t1);
...
//To print the top element in the stack
void TopElement(void)
{
	if(stackPointer > 0)
		printf("TopElement : %lf\n", stack[stackPointer - 1]);
	else
...
void performMathOperation(char localBuffer[])
{
    if(strcmp(localBuffer, "sin") == 0)
        push( sin( pop() ) );
    else if(strcmp(localBuffer, "cos") == 0)
        push( cos( pop() ) );
    else if(strcmp(localBuffer, "exp") == 0)
        push( exp( pop() ) );
    else if(strcmp(localBuffer, "pow") == 0)
    {
        double p = pop();
        push( pow( pop(), p ) );
    }
	...
	while((type = getop(localBuffer)) != EOF)
	{
		switch(type)
		{
			case NUMBER:
				...
				break;
            case MATHFUNC:
                performMathOperation(localBuffer);
                break;
			case '+':
                /*
